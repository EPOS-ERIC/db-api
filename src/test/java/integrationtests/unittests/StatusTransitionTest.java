...
/**
 * Test completo per verificare la propagazione degli stati a cascata.
 *
 * Scenari testati:
 * 1. Creazione DRAFT con relazioni → tutte DRAFT
 * 2. DRAFT → SUBMITTED (via overrideStatus) → tutte SUBMITTED
 * 3. DRAFT → SUBMITTED (via DTO status) → tutte SUBMITTED
 * 4. SUBMITTED → PUBLISHED (via overrideStatus) → tutte PUBLISHED + archivio vecchi
 * 5. SUBMITTED → PUBLISHED (via DTO status) → tutte PUBLISHED + archivio vecchi
 * 6. PUBLISHED → DRAFT (nuova versione) → cascade nuove versioni DRAFT
 * 7. PUBLISHED → ARCHIVED → tutte ARCHIVED
 *
 * Per ogni scenario si verifica:
 * - Status dell'entità principale
 * - Status delle entità collegate (Distribution)
 * - Corretta archiviazione delle vecchie versioni PUBLISHED
 * - Coerenza dei metaId/instanceId
 */
@TestMethodOrder(OrderAnnotation.class)
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class StatusTransitionTest extends TestcontainersLifecycle {
